...
for ($i = 0; $i -lt $lines.Count; $i++) {
    if ($lines[$i] -match 'function buildOffscreens\(\)\{') {
        $startIdx = $i
    }
    if ($startIdx -ge 0 -and $i -gt $startIdx -and $lines[$i] -match '^\s+\}\s*$') {
        if ($lines[$i+1] -match 'function renderBase\(\)\{') {
            $endIdx = $i
            break
        }
    }
...
if ($startIdx -ge 0 -and $endIdx -ge 0) {
    $newCode = @(
        ...
        '        if (loaded === 2) { window.__imagesWaiting = false; buildOffscreens(); renderBase(); }',
        '      }',
        '      return;',
        '    }',
        '',
    ...
    Write-Output "ERROR: Could not find buildOffscreens block."
}
